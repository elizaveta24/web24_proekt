...
<div class="col-lg-4 mx-auto">
    ...
    @endif
	<h2>Отредактировать курс</h2>
    <form method="POST" action="{{route('course.update',$courses)}} ">
    @csrf
	 @method('PATCH')
        <div class="form-group">
            <label for="course-title">Название</label>
            <input type="text" name="title" 
                   value="{{ $courses->title}}" class="form-control" id="course-title">
        </div>
		<div class="form-group">
            <label for="course-cost">Цена</label>
            <input type="text" name="cost" 
                   value="{{ $courses->cost}}" class="form-control" id="course-cost">
        </div>
